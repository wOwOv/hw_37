...
//소켓 셋 초기화
	...
	timevalue.tv_usec = 0;
	//리슨소켓 select
	slretval = select(0, &readset, NULL, NULL, &timevalue);
	...
	FD_ZERO(&writeset);
	int setcnt = 0;
	unordered_map<unsigned int, Player*>::iterator opit;
	unordered_map<unsigned int, Player*>::iterator edit;
	//64개씩 세팅 후 select
	for (unordered_map<unsigned int, Player*>::iterator it = PlayerMap.begin(); it != PlayerMap.end(); it++)
	{
		...
		edit = it;
		++edit;
		//64개 꽉 찼거나 마지막 걸 세팅했다면 select 후 recvproc writeproc하기
		if (setcnt == 64||edit==PlayerMap.end())
		{
			setcnt = 0;
			...
			if (slretval > 0)
			{
				...
				unordered_map<unsigned int, Player*>::iterator iter = opit;
				for (; iter != edit; iter++)
				{
					...
					if (FD_ISSET(tgtply->session->sock, &readset))
					{
						...
						{
							--slretval;
							tgtply->session->LastRecvTime = timeGetTime();
							RecvProc(tgtply);
						}
					}
					//writeset에 있는지 확인
...
void AcceptProc()
{
	...
	if (player->session->sock == INVALID_SOCKET)
	{
		...
		_LOG(LOG_LEVEL_DEBUG, L"accept error: %d\n", atretval);
	}
	else
	{
		player->id = PlayerID;
		...
		player->x = rand() % 6400;
		player->y = rand() % 6400;
		//player->x = rand() % 500 + 50;
		//player->y = rand() % 300 + 100;
		//player->x = 0;
		//player->y = 0;
		player->hp = 100;
		player->move = -1;
		...
		for (int i = 0; i < sectoraround.count; i++)
		{
			...
			for (; it != Sector[sectoraround.around[i].y][sectoraround.around[i].x].end(); it++)
			{
				...
				//움직이고 있었다면 movestart까지
				if (exist->move != -1)
				{
					buf.Clear();
					mpSCMOVESTART(exist->id, exist->move, exist->x, exist->y, &buf);
					SendUnicast(player->session, &buf);
				}
			}
		}
...
void RecvProc(Player* player)
{
	...
	if (player->remove == false)
	{
		...
		if (rvretval == SOCKET_ERROR)
		{
			...
			_LOG(LOG_LEVEL_DEBUG, L"recv error: %d", rvretval);
			Disconnect(player);
		}
		else if (rvretval == 0)
		{
