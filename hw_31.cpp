...
void Update(WORD T)
{
	...
	for (; moveit != PlayerMap.end(); moveit++)
	{
		...
		if (player->move != -1)
		{
			...
			//좌표가 한계를 넘어버리면 고치기
			if (player->x < dfRANGE_MOVE_LEFT)
			{
				player->x = 0;
			}
			if (player->x >= dfRANGE_MOVE_RIGHT)
			{
				player->x = dfRANGE_MOVE_RIGHT - 1;
			}
			...
			{
				player->y = dfRANGE_MOVE_BOTTOM - 1;
			}
		...
		//sector 계산
			...
			if (player->OldSector.x != player->CurSector.x || player->OldSector.y != player->CurSector.y)
			{
				...
				//섹터가 변경된 것이니 그에 따라 player한테 removesec에 있는 객체들 삭제메시지 보내기
				...
				//removesec에 있는 애들에게도 player 삭제하라고 메시지 보내기
				SBuffer delbuf;
				mpSCDELETE(player->id, &delbuf);
				SendSectorList(player->session, &removesec, &delbuf, false);

				//섹터가 변경된 것이니 그에 따라 player한테 addsec에 있는 객체들 생성메시지 보내기
				for (int i = 0; i < addsec.count; i++)
				{
					...
					for (; addit != Sector[addsec.around[i].y][addsec.around[i].x].end(); addit++)
					{
						...
						if (tgtply != player)
						{
							...
							//tgtply가 이동중이었다면 movestart메시지까지..
							if (tgtply->move != -1)
							{
								SBuffer movbuf;
				...
				//addsec에 있는 애들에게도 player생성하고 movestart메시지 보내주기
				SBuffer crtbuf;
				mpSCCREATEOTHER(player->id, player->direction, player->x, player->y, player->hp, &crtbuf);
				SendSectorList(player->session, &addsec, &crtbuf, false);
				
				SBuffer movbuf;
				mpSCMOVESTART(player->id, player->move, player->x, player->y, &movbuf);
				SendSectorList(player->session, &addsec, &movbuf, false);
